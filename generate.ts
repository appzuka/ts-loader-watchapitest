...
const busyWork = 2;  // The number of times to include busyLines in each file

const iterate = (i, f) => {
  return Array(i).fill(0).map((v,i) => f(i)).join('\n')
}
...
const nestedLoopOperation = (counters, depths, level) => {
    if (level === counters.depths) {
      filesGenerated += 1;
      fs.writeFileSync(`${baseDir}/generated_${counters.join('_')}.tsx`, 
      `import React from 'react';
      
      ${iterate(busyWork, () => busyLines)}
      
      export function Generated_${counters.join('_')}() {
        return <h2>Generated component ${counters.join('_')}</h2>;
      }
      `)
    } else {
